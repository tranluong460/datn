...
const roomTypeValidate = joi.object({
  ...
  adults: joi.number().min(0).required().messages(validationErrors("Số lượng người lớn")),
  children: joi.number().min(0).required().messages(validationErrors("Số lượng trẻ em")),
  bed: joi.number().min(1).required().messages(validationErrors("Số lượng giường")),
